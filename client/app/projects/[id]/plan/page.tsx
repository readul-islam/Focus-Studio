...
export default function ProjectPlanPage() {
    ...
    const phases = useMemo(() => {
        ...
        })).filter(p => p.startDate && p.endDate); // Filter invalid phases ? Or keep them?
        // User data shows some phases might have null dates? "Tasks" have null dates in example.
        // Phases seem to have dates. Keep filter for now or handle nulls.
    }, [tasksData]);
    ...
    // Initialize expanded state for all phases on load
    // Initialize expanded state for all phases on load
    ...
    const renderTimeline = () => {
        ...
        if (view === 'week') {
            // Week View: Show Weeks of the Current Month (or broader range?)
            // User request: "total week for the current selected month"
            // Let's frame it as: Start of Month -> End of Month, broken into Weeks.

            rangeStart = startOfWeek(startOfMonth(currentDate), { weekStartsOn: 0 }); // Sunday start as per mock? Nov 30 was Sunday in 2025? (Actually 2025 Nov 30 is Sunday).
            // Let's stick to Sunday start to match "Nov 30" (Sun) -> "Dec 7" (Sun)
            ...
            // Visual trick: Columns are still months, but headers grouped? 
            // Or columns are months.
        ...
        return (
                                            ...
                                            return quarters.map((qCols, qIdx) => {
                                                ...
                                                return (
                                                    <div key={qIdx} className="flex flex-col border-r border-gray-200">
                                                        <div className="h-8 flex items-center justify-center text-xs font-medium text-gray-500 bg-white border-b border-gray-100">
                                                            Q{qNum}
                                                        </div>
            ...
            //  <div className="bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col h-full overflow-hidden min-h-[600px]">
