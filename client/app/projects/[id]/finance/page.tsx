...
function FinancePageContent(params) {
  ...
  } = useFetch(`finance/project-finance?project_id=${id}`, {
    enabled: !!id,
  });
  const { data: projectData, isLoading: projectLoading, refetch } = useFetch(`projects/projects/${id}`, { enabled: !!id });
  const { currency, isLoading: currencyLoading } = useCurrency(projectData?.currency);
  ...
  const { mutate: deleteInvoice } = useDeleteData({
    onSuccess: () => {
      ...
      queryClient.refetchQueries({
        queryKey: [`finance/project-finance/?project_id=${id}`],
      });
    },
    onError: (error: Error) => {
  ...
  const typeFilter: 'All' | 'PO' | 'Invoice' =
    rawType === 'PO' || rawType === 'Invoice' ? rawType : 'All';
  ...
  const safeIncludes = (value: any, search: string) => typeof value === 'string' && value.toLowerCase().includes(search.toLowerCase());
  ...
  const selectedPoIds = useMemo(() => {
    return checkedItems.filter((item: any) => item.display_po).map((item: any) => item.id);
  }, [checkedItems]);

  // Get only Invoice ids from checked items
  const selectedInvoiceIds = useMemo(() => {
    return checkedItems.filter((item: any) => item.display_invoice).map((item: any) => item.id);
  }, [checkedItems]);
    ...
    <div className="">
      <div className="space-y-6">
        ...
        <div className="flex items-center  flex-wrap justify-center lg:justify-between gap-3">
            ...
            {(['All', 'PO', 'Invoice'] as const).map(type => (
              <button
                ...
                className={`relative px-3 py-1.5 text-sm rounded-md transition-colors ${
                  typeFilter === type ? 'text-neutral-900 font-medium' : 'text-neutral-600 hover:text-neutral-900'
                }`}
              >
                {typeFilter === type && (
                  <motion.div
                    layoutId="project-finance-tab-pill"
          ...
          <motion.div layout transition={{ type: 'spring', stiffness: 300, damping: 26 }} className="flex justify-center lg:justify-start items-center gap-2 flex-wrap">
          ...
          {financePermission &&  <motion.div layout>
              <Button
                    ...
                    Array.from({ length: 5 }).map((_, index) => (
                      <tr key={index} className="hover:bg-stone-50">
                        ...
                        <td className="px-2 pr-4 py-3 text-right">
                          <div className="w-16 h-4 bg-stone-200 rounded animate-pulse mx-auto"></div>
                        </td>
                        <td className="px-2 pr-4 py-3 text-right">
                          <div className="w-16 h-4 bg-stone-200 rounded animate-pulse ml-auto"></div>
                        </td>
                        <td className="px-2 pr-4 py-3 text-right">
                          <div className="w-10 h-4 bg-stone-200 rounded animate-pulse mx-auto"></div>
                        </td>
                      </tr>
                    ))}
                  <>
                      ...
                      filteredPurchaseOrders.map((po: any) => (
                        <tr key={po.display_po} className="hover:bg-stone-50">
                          <td className="px-3 py-3">
                            <Checkbox
                            disabled={!financePermission}
                              checked={checkedItems.some(item => item.display_po === po.display_po)}
                          ...
                          <td className="px-3 py-3 text-gray-600 truncate whitespace-nowrap">{po.project?.project_name || '—'}</td>
                          ...
                          <td className="px-3 py-3">
                            <StatusBadge
                              status={po.status}
                              label={STATUS_OPTIONS.find(o => o.value === po.status)?.label || po.status}
                      ...
                      filteredInvoices.map((inv: any) => (
                        <tr key={inv.display_invoice} className="hover:bg-stone-50">
                          <td className="px-3 py-3">
                            <Checkbox
                            disabled={!financePermission}
                              checked={checkedItems.some(item => item.display_invoice === inv.display_invoice)}
                          ...
                          <td className="px-3 py-3 text-gray-600 whitespace-nowrap">{inv.project?.project_name || '—'}</td>
                          <td className="px-3 py-3 text-gray-600 whitespace-nowrap">
