      ...
      {!isLoading && data && (
        <article className="space-y-6">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-500 pb-6 border-b border-gray-100">
            <span>
              By <span className="font-medium text-gray-700">{data.created_by_name}</span>
            </span>
            {data.updated_at !== data.created_at && (
              <span>
                Updated {format(parseISO(data.updated_at), 'MMM d, yyyy')}
              </span>
            )}
          </div>
      ...
      {!isLoading && !data && (
        <div className="py-20 text-center">
          ...
          <p className="text-sm text-gray-500 mb-4">
            This changelog entry doesn't exist or has been removed.
          </p>
          <Link
            href="/changelog"
