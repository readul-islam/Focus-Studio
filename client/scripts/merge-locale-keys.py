...
def main() -> None:
    patch_en = load_merged_patch(PATCH_FILES_EN)
    if not patch_en:
        print("No English patch files found.", file=sys.stderr)
        sys.exit(1)
    merge_file(ROOT / "messages" / "en-US.json", patch_en)
    patch_ja = load_merged_patch(PATCH_FILES_JA) or patch_en
    merge_file(ROOT / "messages" / "ja-JP.json", patch_ja)
    print("Done.")
