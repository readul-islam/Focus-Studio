...
export function DashboardSidebar() {
  ...
  const sidebarItems = useMemo(() => menuItems, []);
  ...
  return (
    <div
      ...
      <div className={cn('p-3 bg-white', isCollapsed && 'px-2')}>
        <div className="flex items-center gap-3 min-h-[44px]">
          ...
          />
          {!isCollapsed && (
            <span className="font-semibold text-gray-900 tracking-tight">{tApp('name')}</span>
          )}
        </div>
      </div>
            ...
            <DropdownMenuTrigger asChild>
              <button className="w-full flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-gray-100 transition-colors text-left focus:outline-none">
                ...
                <span className="flex-1 text-xs font-semibold text-gray-800 truncate">
                  {project?.project_name ?? tTop('noProject')}
                </span>
      ...
      <div className={cn('p-4 bg-white', isCollapsed && 'px-2')}>
        {!isCollapsed && <div className="space-y-1 mb-4" />}
